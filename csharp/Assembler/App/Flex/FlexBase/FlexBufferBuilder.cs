...
namespace Arshu.App.Flex
{
    ...
    //delegate void MapAction(IFlexBufferMapBuilder map);
    //delegate void VectorAction(FlexBufferVectorBuilder vector);
    ...
    public interface IFlexBufferMapBuilder
    {
        ...
        void Add(string key, FlxMap value);
    }
    ...
    internal struct FlexBufferMapBuilder : IFlexBufferMapBuilder
    {
        ...
        public int KeyCount()
        {
            return _buffer.KeyCount();
        }
        
        public void Add(string key, FlxValue flxValue)
        {
            _buffer.AddKey(key);
            if (flxValue.ValueType == FlexType.Null)
            {
                _buffer.AddNull();
            }
            else if (flxValue.ValueType == FlexType.Blob)
            {
                _buffer.Add(flxValue.AsBlob);
            }
            else if (flxValue.ValueType == FlexType.Bool)
            {
                _buffer.Add(flxValue.AsBool);
            }
            else if (flxValue.ValueType == FlexType.Float)
            {
                _buffer.Add(flxValue.AsDouble);
            }
            else if (flxValue.ValueType == FlexType.Int)
            {
                _buffer.Add(flxValue.AsLong);
            }
            else if (flxValue.ValueType == FlexType.String)
            {
                _buffer.Add(flxValue.AsString);
            }
            else if (flxValue.ValueType == FlexType.Map)
            {
                var start = _buffer.StartVector();
                var builder = new FlexBufferMapBuilder(_buffer);
                foreach (var item in flxValue.AsMap)
                {
                    builder.Add(item.Key, item.Value);
                }
                _buffer.SortAndEndMap(start);
            }
            else if (flxValue.ValueType == FlexType.Vector)
            {
                var start = _buffer.StartVector();
                var builder = new FlexBufferVectorBuilder(_buffer);
                foreach (var item in flxValue.AsVector)
                {
                    builder.Add(item);
                }
                _buffer.EndVector(start, false, false);
            ...
            {
                throw new Exception("Value Type Cannot be added [" + flxValue.ValueType + "]");
            }
        }

        public void Add(string key, FlxMap flxMap)
        {
            _buffer.AddKey(key);
            var start = _buffer.StartVector();
            var builder = new FlexBufferMapBuilder(_buffer);
            foreach (var item in flxMap)
            {
                builder.Add(item.Key, item.Value);
            }
            _buffer.SortAndEndMap(start);
