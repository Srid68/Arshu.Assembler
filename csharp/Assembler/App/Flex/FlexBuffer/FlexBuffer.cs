...
namespace Arshu.App.Flex
{
    public class FlexBuffer
    {
        ...
        internal FlexType AddNull()
        {
            _stack.Add(StackValue.Null());
            return FlexType.Null;
        }

        internal FlexType Add(long value)
        {
            _stack.Add(StackValue.Value(value));
            return FlexType.Int;
        }

        internal FlexType AddIndirect(long value)
        {
            var type = FlexType.IndirectInt;
        ...
        internal FlexType Add(ulong value)
        {
            _stack.Add(StackValue.Value(value));
            return FlexType.Uint;
        }

        internal FlexType AddIndirect(ulong value)
        {
            var type = FlexType.IndirectUInt;
            var bitWidth = BitWidthUtil.Width(value);
            var byteWidth = Align(bitWidth);
            var valueOffset = _offset;
            Write(value, byteWidth);
        ...
        internal FlexType Add(double value)
        {
            _stack.Add(StackValue.Value(value));
            return FlexType.Float;
        }

        internal FlexType AddIndirect(double value)
        {
            var type = FlexType.IndirectFloat;
            var bitWidth = BitWidthUtil.Width(value);
            var byteWidth = Align(bitWidth);
            var valueOffset = _offset;
            Write(value, byteWidth);
        ...
        internal FlexType Add(bool value)
        {
            _stack.Add(StackValue.Value(value));
            return FlexType.Bool;
        }

        internal FlexType Add(string value)
        {
            ...
            {
                _stack.Add(StackValue.Value(_stringCache[value], bitWidth, FlexType.String));
                return FlexType.String;
            }
            var byteWidth = Align(bitWidth);
            ...
            _offset = newOffset;
            _stack.Add(StackValue.Value(stringOffset, bitWidth, FlexType.String));
        ...
        internal FlexType Add(byte[] value)
        {
            ...
            _offset = newOffset;
            _stack.Add(StackValue.Value(blobOffset, bitWidth, FlexType.Blob));
            return FlexType.Blob;
        }
        ...
        {
            if (v1.TypeOfValue != FlexType.Key || v2.TypeOfValue != FlexType.Key)
            {
        ...
        private FlexType AddDynamic(object? value)
        {
            switch (value)
            {
                case null:
                ...
                case IEnumerable l:
                    AddDynamicVector(l);
                    return FlexType.Vector;
                default:
                    throw new Exception($"Unexpected type of {value}");
            }
        }

        internal bool HasKey(string value)
        {
            bool ret = false;
            if (_keyCache.ContainsKey(value) == true)
            {
                ret = true;
            }
            return ret;
        ...
        internal void AddKey(string value)
        {
            ...
            {
                _stack.Add(StackValue.Value(_keyCache[value], BitWidth.Width8, FlexType.Key));
                return;
            }
            var bytes = Encoding.UTF8.GetBytes(value);
            var length = (ulong)bytes.Length;
            ...
            Buffer.BlockCopy(bytes, 0, _bytes, (int)_offset, (int)length);
            _offset = newOffset;
            _stack.Add(StackValue.Value(keyOffset, BitWidth.Width8, FlexType.Key));
        ...
        private StackValue CreateVector(int start, int vecLen, int step, bool typed, bool fix, StackValue? keys = null)
        {
            ...
            if (keys != null)
            {
                return StackValue.Value(vloc, bitWidth, FlexType.Map);
            }
            ...
            return StackValue.Value(vloc, bitWidth, FlexType.Vector);
        }
    }
