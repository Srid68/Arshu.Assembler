...
// NormalizeFileContent normalizes file content by removing UTF-8 BOM and normalizing line endings to LF (\n)
func NormalizeFileContent(content string) string {
	if len(content) == 0 {
		return content
	}

	// Remove UTF-8 BOM is '\uFEFF' (U+FEFF) which is 3 bytes: 0xEF 0xBB 0xBF
	if len(content) >= 3 && content[0] == 0xEF && content[1] == 0xBB && content[2] == 0xBF {
		content = content[3:]
	} else if strings.HasPrefix(content, "\uFEFF") {
		// Also handle if BOM is present as Unicode character
		content = content[len("\uFEFF"):]
	}
