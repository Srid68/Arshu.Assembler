...
func (e *EnginePreProcess) MergeTemplates(appSite, appFile, appView string, preprocessedTemplates map[string]model.PreprocessedTemplate, searchAppSites string, enableJsonProcessing bool) string {
	Logger.Debug(fmt.Sprintf("MergeTemplates called: appSite=%s, appFile=%s, appView=%s, searchAppSites=%s, enableJson=%t", appSite, appFile, appView, searchAppSites, enableJsonProcessing), "EnginePreProcess")
	...
	mainPreprocessed := e.GetTemplate(appSite, appFile, preprocessedTemplates, searchAppSites, appView, e.AppViewPrefix, true)
	...
	contentHtml = e.ApplyTemplateReplacements(contentHtml, preprocessedTemplates, enableJsonProcessing, searchAppSites, appView, mainPreprocessed)
...
func (e *EnginePreProcess) GetTemplate(appSite, templateName string, preprocessedTemplates map[string]model.PreprocessedTemplate, searchAppSites, appView, appViewPrefix string, useAppViewFallback bool) *model.PreprocessedTemplate {
	if len(preprocessedTemplates) == 0 {
		return nil
	}
	...
	if searchAppSites != "" {
		...
		for _, searchAppSite := range searchAppSitesArray {
			...
			if tmpl, ok := preprocessedTemplates[searchKey]; ok {
				Logger.Debug(fmt.Sprintf("Template '%s' not found in '%s', using fallback from '%s'", templateName, appSite, searchAppSite), "EnginePreProcess")
				return &tmpl
			}
		}
	}
...
func (e *EnginePreProcess) ApplyTemplateReplacements(content string, preprocessedTemplates map[string]model.PreprocessedTemplate, enableJsonProcessing bool, searchAppSites, appView string, mainTemplate *model.PreprocessedTemplate) string {
	result := content
		...
		if mainTemplate != nil && currentPass == 1 && enableJsonProcessing {
			for _, mapping := range mainTemplate.ReplacementMappings {
				if mapping.Type == model.JsonPlaceholderType && strings.Contains(result, mapping.OriginalText) {
					Logger.Debug(fmt.Sprintf("Applying main template JSON placeholder (original size: %d, replacement size: %d)", len(mapping.OriginalText), len(mapping.ReplacementText)), "EnginePreProcess")
		...
		for _, tmpl := range preprocessedTemplates {
			...
			for _, mapping := range tmpl.ReplacementMappings {
				if mapping.Type == model.SimpleTemplateType && strings.Contains(result, mapping.OriginalText) {
					replacementText := e.ApplyAppViewLogicToReplacement(mapping.OriginalText, mapping.ReplacementText, preprocessedTemplates, searchAppSites, appView)
...
func (e *EnginePreProcess) ApplyAppViewLogicToReplacement(originalText, replacementText string, preprocessedTemplates map[string]model.PreprocessedTemplate, searchAppSites, appView string) string {
	Logger.Debug(fmt.Sprintf("ApplyAppViewLogicToReplacement: originalText=%s, hasReplacementText=%t, appView=%s, searchAppSites=%s",
		originalText, replacementText != "", defaultIfEmpty(appView, "null"), searchAppSites), "EnginePreProcess")
	...
	appViewTemplate := e.GetTemplate(appSite, placeholderName, preprocessedTemplates, searchAppSites, appView, e.AppViewPrefix, true)
