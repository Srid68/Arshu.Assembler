...
class PerformanceUtils {
    ...
    static async runPerformanceComparison(scenarios, enableJsonProcessing = true, skipDetails = false, progressCallback = null) {
        ...
        for (let i = 0; i < scenarios.length; i++) {
            ...
            try {
                ...
                const normalTimeMicros = Math.round(normalTimeMs * 1000); // Convert to microseconds for precision
                ...
                const preProcessTimeMicros = Math.round(preProcessTimeMs * 1000); // Convert to microseconds for precision
                ...
                const scenarioTotalTimeMs = Date.now() - scenarioStartTime;
                const elapsedTimeMs = Date.now() - startTime;

                if (!skipDetails) {
                    console.log(`[ClientJS] Scenario Total Time: ${scenarioTotalTimeMs}ms | Elapsed: ${elapsedTimeMs}ms`);
                }
                ...
                perfSummaryRows.push({
                    ...
                    NormalTimeMicros: normalTimeMicros,
                    PreProcessTimeMicros: preProcessTimeMicros,
                    ...
                    ScenarioTotalTimeMs: scenarioTotalTimeMs,
    ...
    static formatPerfSummaryTable(summaryRows) {
        ...
        html += '<th style="border: 1px solid #ddd; padding: 12px;">ScnTime(ms)</th>';
        html += '<th style="border: 1px solid #ddd; padding: 12px;">Elapsed(ms)</th>';
        html += '</tr>';
        html += '</thead>';
        html += '<tbody>';
        ...
        for (let i = 0; i < summaryRows.length; i++) {
            ...
            html += `<td style="border: 1px solid #ddd; padding: 12px;">${row.ScenarioTotalTimeMs || 0}</td>`;
            html += `<td style="border: 1px solid #ddd; padding: 12px;">${row.ElapsedTimeMs || 0}</td>`;
            html += '</tr>';
        }
