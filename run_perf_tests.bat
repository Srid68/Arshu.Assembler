...
REM cd "./csharp/AssemblerTest" && dotnet run -c Release --skipdetails && cd ../../
...
rd /s/q "%~dp0csharp\AssemblerTest\template_analysis"
cd "%~dp0csharp\AssemblerTest" && dotnet run -c Release -- --skipdetails && cd "%~dp0"
rem pause

REM cd "./rust/AssemblerTest" && cargo run --release -- --skipdetails && cd ../../
...
rd /s/q "%~dp0rust\AssemblerTest\template_analysis"
cd "%~dp0rust\AssemblerTest" && cargo run --release -- --skipdetails && cd "%~dp0"

REM cd "./go/AssemblerTest" && go run . --skipdetails && cd ../../
...
rd /s/q "%~dp0go\AssemblerTest\template_analysis"
cd "%~dp0go\AssemblerTest" && go run . --skipdetails && cd "%~dp0"

REM cd "./node/AssemblerTest" && node index.js --skipdetails && cd ../../
...
rd /s/q "%~dp0node\AssemblerTest\template_analysis"
cd "%~dp0node\AssemblerTest" && node index.js -- --skipdetails && cd "%~dp0"

REM cd "./php/AssemblerTest" && php index.php --skipdetails && cd ../../
...
rd /s/q "%~dp0php\AssemblerTest\template_analysis"
cd "%~dp0php\AssemblerTest" && php index.php -- --skipdetails && cd "%~dp0"

REM JavaScript Client-Side Tests
echo.
echo Running JavaScript client-side performance tests...
rd /s/q "%~dp0csharp\AssemblerWebJs\template_analysis"
powershell -ExecutionPolicy Bypass -File "%~dp0run_perf_tests.ps1"
