...
impl ApiResponse {
    ...
    fn serialize_slot_placeholder(slot: &crate::model::model_preprocess::SlotPlaceholder) -> String {
        let mut sb = String::new();
        sb.push('{');
    ...
    pub fn create_preprocessed_summary(site_templates: &PreprocessedSiteTemplates) -> PreprocessedSummary {
        PreprocessedSummary {
            site_name: site_templates.site_name.clone(),
            templates_requiring_processing: site_templates.templates.values()
                .filter(|t| t.requires_processing())
                .count(),
            templates_with_json_data: site_templates.templates.values()
                .filter(|t| t.has_json_data())
                .count(),
            templates_with_placeholders: site_templates.templates.values()
