...
impl EnginePreProcess {
    ...
    pub fn merge_templates(
        ...
        Logger::debug(&format!("MergeTemplates called: appSite={}, appFile={}, appView={}, searchAppSites={}, enableJson={}",
            app_site, app_file, app_view_str, search_app_sites, enable_json_processing), Some("EnginePreProcess"));
        ...
        let result = self.apply_template_replacements(
            &content_html,
            preprocessed_templates,
    ...
    fn get_template<'a>(
        ...
        if !search_app_sites.is_empty() {
            ...
            for search_app_site in search_app_sites_array {
                ...
                if let Some(search_template) = preprocessed_templates.get(&search_key) {
                    Logger::debug(
                        &format!(
    ...
    fn apply_template_replacements(
        ...
        loop {
            ...
            if current_pass == 1 && enable_json_processing {
                for mapping in main_template
                    .replacement_mappings
                    .iter()
                    .filter(|m| matches!(m.r#type, ReplacementType::JsonPlaceholder))
                {
                    if result.contains(&mapping.original_text) {
                        Logger::debug(&format!("Applying main template JSON placeholder (original size: {}, replacement size: {})", mapping.original_text.len(), mapping.replacement_text.len()), Some("EnginePreProcess"));
            ...
            for template in preprocessed_templates.values() {
                ...
                {
                    if result.contains(&mapping.original_text) {
                        Logger::debug(&format!("Applying slotted template (original size: {}, replacement size: {})", mapping.original_text.len(), mapping.replacement_text.len()), Some("EnginePreProcess"));
                        result = result.replace(&mapping.original_text, &mapping.replacement_text);
                        slotted_count += 1;
                    }
                }
    ...
    fn apply_app_view_logic_to_replacement(
        ...
        Logger::debug(&format!("ApplyAppViewLogicToReplacement: originalText={}, hasReplacementText={}, appView={}, searchAppSites={}",
            original_text, !replacement_text.is_empty(), app_view.unwrap_or("null"), search_app_sites), Some("EnginePreProcess"));
        ...
        if app_view.is_none() || app_view == Some("") {
            Logger::debug(
                "No appView context, returning replacementText",
                Some("EnginePreProcess"),
            );
        ...
        let app_view_template = app_view_template.unwrap();
        ...
        let mut processed_content = app_view_template.original_content.clone();
        for json_mapping in app_view_template
            .replacement_mappings
            .iter()
            .filter(|m| matches!(m.r#type, ReplacementType::JsonPlaceholder))
        {
