...
impl LoaderPreProcess {
    pub fn load_process_get_template_files(root_dir_path: &str, app_site: &str) -> PreprocessedSiteTemplates {
        Logger::debug(&format!("LoadProcessGetTemplateFiles called for appSite: {}", app_site), Some("LoaderPreProcess"));
...

        {
            ...
            if let Some(cached) = cache.get(&cache_key) {
                Logger::debug(&format!("Returning cached templates for {} ({} templates)", app_site, cached.templates.len()), Some("LoaderPreProcess"));
                return cached.clone();
            }
        }
        ...
        if !Path::new(&app_sites_path).exists() {
            Logger::warn(&format!("AppSites directory not found: {}", app_sites_path), Some("LoaderPreProcess"));
            let mut cache = PREPROCESSED_TEMPLATES_CACHE.lock().unwrap();
            cache.insert(cache_key, result.clone());
            return result;
        }

        Logger::debug(&format!("Loading templates from: {}", app_sites_path), Some("LoaderPreProcess"));
            ...
            .filter_map(|e| e.ok()) 
        {
            ...
            if path.extension().map(|ext| ext == "html").unwrap_or(false) {
                ...
                let content = CommonUtil::normalize_file_content(&fs::read_to_string(path).unwrap_or_default());

                Logger::debug(&format!("Loading template: {} (size: {})", key, content.len()), Some("LoaderPreProcess"));
                ...
                let json_content = if json_file.exists() {
                    ...
                    Logger::debug(&format!("Found JSON file for {} (size: {})", key, json_str.len()), Some("LoaderPreProcess"));
                    Some(json_str)
                } else {
                        ...
                        if let Ok(entries) = fs::read_dir(parent_dir) {
                            ...
                            if let Some(json_entry) = matching_json {
                                if let Ok(json_str) = fs::read_to_string(json_entry.path()) {
                                    ...
                                    Logger::debug(&format!("Found JSON file (case-insensitive) for {} (size: {})", key, json_str.len()), Some("LoaderPreProcess"));
                                    Some(json_str)
                                } else {
                                    None
                                }
                ...
                Logger::debug(&format!("Preprocessed {}: {} replacements, {} slotted, {} placeholders",
                    key, preprocessed.replacement_mappings.len(), preprocessed.slotted_templates.len(), preprocessed.placeholders.len()), Some("LoaderPreProcess"));
    ...
    fn create_all_replacement_mappings_for_site(site_templates: &mut PreprocessedSiteTemplates, app_site: &str) {
        ...
        Logger::debug(&format!("Creating replacement mappings for {} - Phase 1: JSON arrays", app_site), Some("LoaderPreProcess"));
        ...
        Logger::debug(&format!("Creating replacement mappings for {} - Phase 2: Simple placeholders", app_site), Some("LoaderPreProcess"));
        ...
        Logger::debug(&format!("Creating replacement mappings for {} - Phase 3: Slotted templates", app_site), Some("LoaderPreProcess"));
        let all_templates_snapshot = site_templates.templates.clone();
        for key in &template_keys {
            if let Some(template) = site_templates.templates.get_mut(key) {
    ...
    fn create_slotted_template_replacement_mappings(template: &mut PreprocessedTemplate, all_templates: &HashMap<String, PreprocessedTemplate>, app_site: &str) {
        ...
        for slotted_template in &template.slotted_templates {
            ...
            if let Some(target_template) = all_templates.get(&target_template_key) {
                ...
                processed_template = CommonUtil::remove_remaining_slot_placeholders(&processed_template);
    ...
    fn parse_slotted_templates(content: &str, template: &mut PreprocessedTemplate) {
        ...
        while search_pos < content.len() {
            ...
            let close_start = match CommonUtil::find_matching_close_tag(content, open_end + 2, &open_tag, &close_tag) {
                Some(pos) => pos,
                None => {
                    search_pos = open_start + 1;
                    continue;
    ...
    fn parse_slots(inner_content: &str, slots: &mut Vec<SlotPlaceholder>) {
        ...
        while search_pos < inner_content.len() {
            ...
            let close_start = match CommonUtil::find_matching_close_tag(inner_content, slot_open_end, &open_tag, &close_tag) {
                Some(pos) => pos,
                None => {
                    search_pos = slot_start + 1;
                    continue;
