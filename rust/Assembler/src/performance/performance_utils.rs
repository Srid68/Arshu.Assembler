...
impl PerformanceUtils {
    ...
    pub fn run_performance_comparison(
        ...
        for scenario in scenarios {
            ...
            let app_view_prefix = if app_view.is_empty() {
                ...
                app_view
                    .chars()
                    .take(std::cmp::min(app_view.len(), 6))
                    .collect::<String>()
            };
            ...
            if !skip_details {
                println!("{}", "-".repeat(60));
                println!(
                    "[Rust] Testing: AppSite={}, AppFile={}, AppView={}",
                    test_app_site, app_file_name, app_view
                );
            ...
            for _ in 0..iterations {
                result_normal = normal_engine.merge_templates(
                    &test_app_site,
                    &app_file_name,
                    if app_view.is_empty() {
                        None
            ...
            if !skip_details {
                ...
                println!(
                    "[Rust] Normal Engine:     {:.0}ms | Avg: {:.3}ms/op | Size: {} chars",
                    normal_time_ms,
                    avg,
            ...
            for _ in 0..100 {
                preprocess_engine.merge_templates(
                    &test_app_site,
                    &app_file_name,
                    if app_view.is_empty() {
                        None
            ...
            for _ in 0..iterations {
                result_preprocess = preprocess_engine.merge_templates(
                    &test_app_site,
                    &app_file_name,
                    if app_view.is_empty() {
                        None
            ...
            if !skip_details {
                ...
                println!(
                    "[Rust] PreProcess Engine: {:.0}ms | Avg: {:.3}ms/op | Size: {} chars",
                    preprocess_time_ms,
                    avg,
            ...
            if !skip_details {
                println!(
                    "[Rust] Scenario Total Time: {}ms | Elapsed: {}ms",
                    scenario_total_time, elapsed_time
                );
            }
            ...
            let perf_diff = if normal_time_ms > 0.0 {
                format!(
                    "{:.1}%",
                    (preprocess_time_ms - normal_time_ms) / normal_time_ms * 100.0
                )
            } else {
                "0%".to_string()
            };
            ...
            perf_summary_rows.push(PerfSummaryRow {
                ...
                results_match: if results_match {
                    "YES".to_string()
                } else {
                    "NO".to_string()
                },
    ...
    pub fn print_perf_summary_table(
        _assembler_web_dir_path: &str,
        project_directory: &str,
        summary_rows: &Vec<PerfSummaryRow>,
    ) {
        ...
        for row in summary_rows {
            ...
            print!(
                "{:<width$.2}",
                format!("{:.2}", row.normal_time_ms()),
                width = widths[2]
            );
            print!(" | ");
            print!(
                "{:<width$.2}",
                format!("{:.2}", row.preprocess_time_ms()),
                width = widths[3]
            );
            print!(" | ");
